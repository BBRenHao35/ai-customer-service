...
有 Infrastructure / DevOps 背景，熟悉 Docker、CI/CD、GCP 等部署流程。這個專案想把這套流程接上 AI 應用，實際跑通一次 RAG 架構——從 Gemini embedding、pgvector 向量搜尋到 Cloud Run 部署上線，並串接 Telegram Bot 作為第二個前端入口。主要透過 [Claude Code](https://claude.ai/code) 輔助開發。
